...
function checkGroupAccess(req, res, next) {
  ...
  try {
    ...
    // 2. Not a member? Check if group is public (no password)
    const group = db.prepare('SELECT name, password_hash FROM groups WHERE id = ?').get(groupId);
    if (!group) return res.status(404).json({ error: 'Group not found.' });

    // Public groups (including 'general') allow auto-join
    ...
    console.error('[auth] checkGroupAccess error:', e);
